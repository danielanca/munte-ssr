...
const ProductView = ({ notifyMe, productQuantity }: CartProps) => {
  ...
  const productQuantityIncrement = () => {
    setProductCountQuantity(productCountQuantity + 1);
  };

  const productQuantityDecrement = () => {
    if (Number(productCountQuantity) > 1) {
      setProductCountQuantity(productCountQuantity - 1);
    }
  };

  
  // const productBulkQuantity = location.state.productCountNumber;
  ...
  const addCartHandler = () => {
    ...
    storedCart.forEach((item) => {
      if (item.id === ID.toString()) {
        item.itemNumber = (Number(item.itemNumber) + Number(Number(productCountQuantity) + 10)).toString();
        itemFound = true;
      }
    });
    if (!itemFound) {
      storedCart.push({ id: ID, itemNumber: Number(productCountQuantity) });
    }
    localStorage.setItem(CartInfoItemCookie, JSON.stringify(storedCart));
